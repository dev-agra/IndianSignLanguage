...
if is_server_side():
    ...
    def _configure_mfa(email, mfa_error, require_password, allow_cancel, confirm_button_text):
        ...
        while True:
            ...
            password_box.set_event_handler("pressed_enter", lambda **e: mfa_panel.raise_event("x-close-alert"))
            ...
            if not selected_mfa_type:
                ...
                if selected_mfa_type == "totp":
                    ...
                    totp_box.set_event_handler("pressed_enter", lambda **e: password_box.focus() if require_password else mfa_panel.raise_event("x-close-alert"))
                    ...
                    if not mfa_methods.get('twilio-verify'):
                        phone_box = TextBox(placeholder="Enter your phone number", align="center")
                        phone_box.set_event_handler("pressed_enter", lambda **e: mfa_panel.raise_event("x-close-alert"))
                        phone_box.set_event_handler("show", lambda **e: phone_box.focus())
                        mfa_panel.add_component(phone_box)
                    else:
                        twilio_box = TextBox(placeholder="Enter 6-digit code", align="center", font="monospace")
                        twilio_box.set_event_handler("pressed_enter", lambda **e: password_box.focus() if require_password else mfa_panel.raise_event("x-close-alert", value=twilio_box.text))
                        twilio_box.set_event_handler("show", lambda **e: twilio_box.focus())
                        mfa_panel.add_component(twilio_box)                            

                        sms_link = Link(text="Resend SMS", icon="fa:commenting-o")
            ...
            if m == 'cancel':
                ...
                if not mfa_methods.get('twilio-verify'):
                    if phone_box.text:
                        mfa_methods['twilio-verify'] = generate_twilio_mfa_method(phone_box.text)
                        send_twilio_token(mfa_methods['twilio-verify'], channel)
                        mfa_error = None
                    else:
                        mfa_error = "Please enter your phone number."
                else:
    ...
    def mfa_login_with_form(email, password):
        ...
        if 'twilio-verify' in mfa_types:
            ...
            def use_phone(**e):
                anvil.server.call("anvil.private.users.twilio.send_verification_token", None, "call")
            def use_sms(**e):
                anvil.server.call("anvil.private.users.twilio.send_verification_token", None, "sms")
            ...
            twilio_box.set_event_handler("show", lambda **e: twilio_box.focus())
            mfa_panel.add_component(Label(text="Please enter the 6-digit code we sent to your phone"))
            mfa_panel.add_component(twilio_box)
